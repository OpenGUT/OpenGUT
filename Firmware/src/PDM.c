...
#define PDM_SAMPLE_RATE 16000U
#define PDM_SAMPLE_WIDTH 16U
#define PDM_MONO_CHANNELS 1U
...
#define PDM_BLOCK_SIZE(channels) \
    (PDM_BYTES_PER_SAMPLE * (channels) * (PDM_SAMPLE_RATE / PDM_BLOCKS_PER_SECOND))
#define PDM_BLOCK_COUNT 4U
#define PDM_RIGHT_FILE_PATH "/SD:/right.wav"
#define PDM_LEFT_FILE_PATH "/SD:/left.wav"
#define PDM_RECORD_ROTATE_COUNT 10U
#define PDM_RECORD_PATH_MAX_LEN 32U
#define PDM_LOOPBACK_TIMEOUT_MS 2000
#define PDM_LOOPBACK_OUTPUT_CHANNELS 2U

K_MEM_SLAB_DEFINE_STATIC(loopback_tx_slab, PDM_BLOCK_SIZE(PDM_STEREO_CHANNELS), PDM_BLOCK_COUNT, 4);

K_MEM_SLAB_DEFINE_STATIC(pdm_mem_slab, PDM_BLOCK_SIZE(PDM_STEREO_CHANNELS), PDM_BLOCK_COUNT, 4);
                              ...
                              uint32_t pcm_data_size)
{
    ...
    header->sample_rate = PDM_SAMPLE_RATE;
    header->byterate = PDM_SAMPLE_RATE * channels * (PDM_SAMPLE_WIDTH / 8U);
                            ...
                            uint32_t pcm_data_size)
{
    ...
    create_wav_header(&header, channels, pcm_data_size);
                             ...
                             uint32_t pcm_data_size)
{
    ...
    ret = write_wav_header(file, channels, pcm_data_size);
    if (ret < 0) {
        return ret;
    }
...
static int pdm_select_stereo_record_path(char *file_path, size_t file_path_size)
{
    struct fs_dirent entry;
    int ret;

    if ((file_path == NULL) || (file_path_size == 0U)) {
        return -EINVAL;
    }

    for (uint32_t index = 0U; index < PDM_RECORD_ROTATE_COUNT; ++index) {
        (void)snprintk(file_path, file_path_size, "/SD:/rec%u.wav", index);
    ...
    (void)snprintk(file_path, file_path_size, "/SD:/rec0.wav");
    return 0;
}
...
static int loopback_i2s_configure(void)
{
    struct i2s_config i2s_cfg = {
        .word_size = PDM_SAMPLE_WIDTH,
        .channels = PDM_LOOPBACK_OUTPUT_CHANNELS,
        .format = I2S_FMT_DATA_FORMAT_I2S,
        .frame_clk_freq = PDM_SAMPLE_RATE,
        .block_size = PDM_BLOCK_SIZE(PDM_STEREO_CHANNELS),
...
static int loopback_queue_block(void *tx_block, bool *started, uint32_t *queued_blocks)
{
    int ret = i2s_write(loopback_i2s_dev, tx_block, PDM_BLOCK_SIZE(PDM_STEREO_CHANNELS));
...
static int pdm_record_file(const char *file_path,
                           ...
                           uint32_t duration_seconds)
{
    ...
    uint32_t block_count = duration_seconds * PDM_BLOCKS_PER_SECOND;
    uint32_t block_size = PDM_BLOCK_SIZE(channel_count);
    uint32_t total_pcm_size = block_count * block_size;
    int ret;
    ...
    ret = write_wav_header(&file, channel_count, total_pcm_size);
    if (ret < 0) {
        (void)fs_close(&file);
        return ret;
    }
    ...
    cfg.streams[0].pcm_rate = PDM_SAMPLE_RATE;
...
{
    if (mic == PDM_MIC_RIGHT) {
        return pdm_record_file(PDM_RIGHT_FILE_PATH,
                               PDM_MONO_CHANNELS,
                               dmic_build_channel_map(0, 0, PDM_CHAN_RIGHT),
                               duration_seconds);
    }

    return pdm_record_file(PDM_LEFT_FILE_PATH,
                           PDM_MONO_CHANNELS,
...
{
    char file_path[PDM_RECORD_PATH_MAX_LEN];
    int ret = SD_Write_Hello_File();

    if (ret < 0) {
        return ret;
    }

    ret = pdm_select_stereo_record_path(file_path, sizeof(file_path));
    if (ret < 0) {
        return ret;
    }

    return pdm_record_file(file_path,
                           PDM_STEREO_CHANNELS,
                           dmic_build_channel_map(0, 0, PDM_CHAN_LEFT) |
...
int PDM_Record_Stereo_Wav_Until(pdm_stop_cb_t stop_cb, void *context)
{
    ...
    ret = write_wav_header(&file, PDM_STEREO_CHANNELS, 0U);
    if (ret < 0) {
        (void)fs_close(&file);
        return ret;
    }
    ...
    stream.block_size = PDM_BLOCK_SIZE(PDM_STEREO_CHANNELS);
    ...
    ret = finalize_wav_file(&file, PDM_STEREO_CHANNELS, total_pcm_size);
    if (ret < 0) {
        (void)fs_close(&file);
        return ret;
    }
...
int PDM_Live_Monitor(uint32_t duration_seconds)
{
    ...
    for (uint32_t block = 0U; block < block_count; ++block) {
        ...
        if (size != PDM_BLOCK_SIZE(PDM_STEREO_CHANNELS)) {
        ...
        ret = loopback_queue_block(tx_block, &started, &queued_blocks);
        if (ret < 0) {
            (void)dmic_trigger(dmic_dev, DMIC_TRIGGER_STOP);
            (void)i2s_trigger(loopback_i2s_dev, I2S_DIR_TX, I2S_TRIGGER_DROP);
            return ret;
